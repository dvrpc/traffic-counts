//! Data structures and functions specific to non-permanent traffic counts.
...
use crate::db::{self, ImportLogEntry};
use crate::{CountError, GetDate};

pub mod check_data;
pub mod extract_from_file;
pub mod intermediate;

use intermediate::{BinnedCountKey, SpeedRangeCount, VehicleClassCount};

/// All of the kinds of non-permanent counts.
///
...
impl FromStr for NonPermCountKind {
    type Err = CountError;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "Bicycle 1" => Ok(NonPermCountKind::Bicycle1),
            "Bicycle 2" => Ok(NonPermCountKind::Bicycle2),
            "Bicycle 3" => Ok(NonPermCountKind::Bicycle3),
            "Bicycle 4" => Ok(NonPermCountKind::Bicycle4),
            "Bicycle 5" => Ok(NonPermCountKind::Bicycle5),
            "Bicycle 6" => Ok(NonPermCountKind::Bicycle6),
            "Pedestrian" => Ok(NonPermCountKind::Pedestrian),
            "Pedestrian 2" => Ok(NonPermCountKind::Pedestrian2),
            "Crosswalk" => Ok(NonPermCountKind::Crosswalk),
            "Volume" => Ok(NonPermCountKind::Volume),
            "15 min Volume" => Ok(NonPermCountKind::FifteenMinVolume),
            "Class" => Ok(NonPermCountKind::Class),
            "Manual Class" => Ok(NonPermCountKind::ManualClass),
            "Speed" => Ok(NonPermCountKind::Speed),
            "8 Day" => Ok(NonPermCountKind::EightDay),
            "Loop" => Ok(NonPermCountKind::Loop),
            "Turning Movement" => Ok(NonPermCountKind::TurningMovement),
            "Video" => Ok(NonPermCountKind::Video),
            _ => Err(CountError::UnknownCountType(s.to_string())),
        }
    }
}

impl Display for NonPermCountKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            NonPermCountKind::Bicycle1 => write!(f, "Bicycle 1"),
            NonPermCountKind::Bicycle2 => write!(f, "Bicycle 2"),
            NonPermCountKind::Bicycle3 => write!(f, "Bicycle 3"),
            NonPermCountKind::Bicycle4 => write!(f, "Bicycle 4"),
            NonPermCountKind::Bicycle5 => write!(f, "Bicycle 5"),
            NonPermCountKind::Bicycle6 => write!(f, "Bicycle 6"),
            NonPermCountKind::Pedestrian => write!(f, "Pedestrian"),
            NonPermCountKind::Pedestrian2 => write!(f, "Pedestrian 2"),
            NonPermCountKind::Crosswalk => write!(f, "Crosswalk"),
            NonPermCountKind::Volume => write!(f, "Volume"),
            NonPermCountKind::FifteenMinVolume => write!(f, "15 min Volume"),
            NonPermCountKind::Class => write!(f, "Class"),
            NonPermCountKind::ManualClass => write!(f, "Manual Class"),
            NonPermCountKind::Speed => write!(f, "Speed"),
            NonPermCountKind::EightDay => write!(f, "8 Day"),
            NonPermCountKind::Loop => write!(f, "Loop"),
            NonPermCountKind::TurningMovement => write!(f, "Turning Movement"),
            NonPermCountKind::Video => write!(f, "Video"),
        }
    }
}
...
/// The full metadata of a non-permanent count, which corresponds to the "tc_header" table in the
/// database.
...
pub struct Metadata {
    ...
    pub count_kind: Option<NonPermCountKind>,
...
mod tests {
    ...
    use super::*;
    use crate::non_perm::extract_from_file::Bicycles;
    use db::{create_pool, get_non_perm_creds};
    ...
    fn hourly_vol_count_correct_num_records_and_total_count_166905() {
        let (username, password) = get_non_perm_creds();
